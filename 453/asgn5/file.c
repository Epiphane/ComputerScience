...
void find_file_r(struct inode *file, struct inode *base, char *path) {
   ...
   if (strlen(path) == 0) {
      ...
      pipe(file_reader);
      if (!fork()) {
         // Read the actual directory file
         close(file_reader[0]);
         read_file(file_reader[1], base);
         exit(0);
      }
      else {
         // Let the reader send me data
         close(file_reader[1]);
         ...
         wait(NULL);
         while (read(file_reader[0], &file_entry, sizeof(struct direntry))) {
            // If inode is 0, then skip it
            if (file_entry.inode == 0)
               continue;

            if (flen == strlen((const char *)file_entry.name) &&
                !strncmp(path, (const char *)file_entry.name, flen)) {
...
void read_file(int fd, struct inode *file) {
   ...
   char *buffer = (char *)malloc(zone_size);
   ...
   while (bytes_left) {
      ...
      if (bytes_to_read) {
         ...
         bytes_read = fread(buffer, 1, bytes_to_read, fs);
